...
score_font = pygame.freetype.Font("media/Tiny5-Regular.ttf", 32)
score = 0
score_position = (0,0)
lives = [0,0,0]
skip_intro = 0
...
while running:
    ...
    if level_transition != 0:
        ...
        if level_transition == 1:
            ...
            screen.blit(background1_list[current_frame])
    ...
    #Draw overlay
    if endless_timer != 0:
        score_img = score_font.render(f"Score: {score}", "#e7ffb7", size=32)
        score_position = (-31 + life_offset[2] + 1024 - score_img[0].get_width(), 31 + life_offset[i])
        if game_over:
            score_position = (512-score_img[0].get_width()/2, 384-score_img[0].get_height()/2)
        screen.blit(score_img[0], score_position)
        for i in range(3):
            if 6 < lives[i] < 9:
                lives[i] += 1/20
            if 9 <= lives[i] <  13:
